...
import { querySpatialStatistics, queryProximity } from 'utils/openspp-api';
import type {
  ...
  reducers: {
    ...
    setProximityParams: (
      ...
      if (payload.lat !== undefined) {
        state.proximityLat = payload.lat;
      }
      if (payload.lng !== undefined) {
        state.proximityLng = payload.lng;
      }
...
export const opensppProximityParamsSelector = createSelector(
  (state: RootState) => state.opensppQueryState.proximityLat,
  (state: RootState) => state.opensppQueryState.proximityLng,
  (state: RootState) => state.opensppQueryState.proximityRadiusKm,
  (state: RootState) => state.opensppQueryState.proximityRelation,
  (lat, lng, radiusKm, relation) => ({ lat, lng, radiusKm, relation }),
);
